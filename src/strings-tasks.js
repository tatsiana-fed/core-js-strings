 ...
 */
function removeFirstOccurrences(string, replace) {
  const i = string.indexOf(replace);
  if (i === -1) {
    return string;
  }
  return string.slice(0, i) + string.slice(i + replace.length);
}
 ...
 */
function removeLastOccurrences(string, replace) {
  const i = string.lastIndexOf(replace);
  if (i === -1) {
    return string;
  }
  return string.slice(0, i) + string.slice(i + replace.length);
...
function countVowels(s) {
  if (typeof s === 'string' || s instanceof String) {
    if (s !== '') {
      const vowels = s.match(/[aeyiou]/gi);
      if (vowels !== null) {
...
function isPalindrome(str) {
  const r = /[\W_]/g;
  const rs = str.toLowerCase().replace(r, '');
  return rs === rs.split('').reverse().join('');
}
...
function encodeToRot13(str) {
  ...
  const I = (x) => input.indexOf(x);
  return str
    .split('')
    .map((x) => (I(x) > -1 ? output[I(x)] : x))
    .join('');
}
